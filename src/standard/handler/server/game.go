...
type GameHandler struct {
	...
	client map[int32][]gamev1.GameService_PlayServer // 状態変更時にクライアントにストリーミングを返すために格納する
...
func NewGameHandler() *GameHandler {
	return &GameHandler{
		...
		client: make(map[int32][]gamev1.GameService_PlayServer),
	}
}

func (h *GameHandler) Play(stream gamev1.GameService_PlayServer) error {
	for {
...
func (h *GameHandler) start(stream gamev1.GameService_PlayServer, roomID int32, me *model.Player) error {
	h.Lock()
	defer h.Unlock()
	...
	if g == nil {
		...
		h.games[roomID] = g
		h.client[roomID] = make([]gamev1.GameService_PlayServer, 0, 2)
	}
