...
func PreRun(t *TicTacToe) error {
	...
	// マッチング問い合わせ
	err = Matching(ctx, matchingv1.NewMatchingServiceClient(conn), t)
	if err != nil {
		return err
	}
	...
	// 双方向ストリーミングでゲーム処理
	return ExecPlay(ctx, gamev1.NewGameServiceClient(conn), t)
}

func Matching(ctx context.Context, client matchingv1.MatchingServiceClient, t *TicTacToe) error {
	// マッチングリクエスト
	stream, err := client.JoinRoom(ctx, &matchingv1.JoinRoomRequest{})
	if err != nil {
		return err
	}
	...
	for {
		...
		if resp.GetStatus() == matchingv1.JoinRoomResponse_STATUS_MATCHED {
			// マッチング成立
			t.Room = adapter.Room(resp.GetRoom())
			t.Me = adapter.Player(resp.GetMe())
			fmt.Printf("Matched room_id=%d\n", resp.GetRoom().GetId())
			return nil
		} else if resp.GetStatus() == matchingv1.JoinRoomResponse_STATUS_WAITTING {
			// 待機中
...
func ExecPlay(ctx context.Context, client gamev1.GameServiceClient, t *TicTacToe) error {
...
func Receive(ctx context.Context, stream gamev1.GameService_PlayClient, t *TicTacToe) error {
	for {
		// サーバーからのストリーミングを受け取る
		res, err := stream.Recv()
		if err != nil {
...
func Send(ctx context.Context, stream gamev1.GameService_PlayClient, t *TicTacToe) error {
	for {
		t.RLock()
